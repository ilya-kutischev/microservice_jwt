...
def create_user(request: Request, user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    if len(user.password.strip()) < 4:
        raise HTTPException(status_code=409, detail="password too short")
...
@app.put("/user/notes")
async def update_user_note(request: Request, access_token: str, note_id: int, title: str, description: str,
                           db: Session = Depends(get_db)):
    email = security.get_current_user_email(access_token)
    db_user = crud.get_user_by_email(db, email=email)
    note = {
